...
class tk_Msginput:
    ...
    def tk_input(self):
                ...
                for row_num in self.row_list:
                    ...
                    if row_num:
                        try:
                            ...
                            break
                        """if self.col["puttype"]==0:
                            if cell[2][:5]=="/cake":#非分润服务器更新
                                continue
                        elif self.col["puttype"]==1:
                            if cell[2][:5]!="/cake":#分润服务器更新
                                continue
                        else:
                            pass"""
        ...
        b6=Button(self.root,text="退出程序",command=(lambda x=self.root:x.destroy()),bd=3,overrelief=FLAT,fg="blue",width=18,height=2).grid(row=13,column=1)
        check_var=IntVar()
