...
class MyFTP():    
    ...
    def ConnectFTP(self,remoteip,remoteport,loginname,loginpassword):    
        try:  
            self.ftp.connect(remoteip,remoteport,600)  
            print 'success' 
        except Exception, e:
            ...
            try:    
                self.ftp.login(loginname,loginpassword)    
                print('login success') 
            except Exception, e:
...
if __name__=="__main__":
    import readfile
    tablist=readfile.readfile("ftptest.csv")
    tabdict=dict(tablist)
    print tabdict
    print tabdict["remoteip"]
    print int(tabdict["remoteport"])
    ftp=MyFTP()
    ftp=ftp.ConnectFTP(tabdict["remoteip"], int(tabdict["remoteport"]), tabdict["loginname"], tabdict["passwd"])
    ftp.quit()