...
def mysql_install():
    ...
    run("sed -i '/\[mysqld\]/a\max_allowed_packet=300M' /etc/my.cnf")
    run("sed -i '/\[mysqld\]/a\max_connections=1500' /etc/my.cnf")
    run("sed -i '/\[mysqld\]/a\lower_case_table_names=1' /etc/my.cnf")
    run("sed -i '/\[mysqld\]/a\skip-name-resolve' /etc/my.cnf")
